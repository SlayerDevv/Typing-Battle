...
const Profile = () => {
    ...
    useEffect(() => {
      const hasChanges = firstName.length > 0 || lastName.length > 0 || userName.length > 0;
    ...
    return (
        ...
        <div className="w-[250px] space-y-1">
            <div 
                ...
                className="w-full text-center cursor-pointer gap-2 flex flex-col justify-center items-center p-2 bg-black/30 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl transform hover:scale-105 transition-transform duration-300"
            >
                ...
                <div className="flex gap-3 justify-center items-center">
                    ...
                    <h2 className="text-yellow-400 w-[200px] font-semibold truncate">
                        {user.firstName} {user.lastName}
                    </h2>
                </div>
            </div>
            <ul className={clsx(
                "w-full text-center flex flex-col justify-center items-center p-3 gap-3 bg-black/30 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl duration-300",
                isOpen ? "" : "hidden"
            )}>
                ...
                <Button
                    ...
                    className="w-full disabled:bg-white/50 flex justify-center group items-center gap-2 bg-yellow-400/5 hover:bg-yellow-400/10 text-yellow-400 px-6 py-3 rounded-xl transition-all duration-200 cursor-pointer backdrop-blur-md"
                >
                    {isLoading ? (
